...
// Log initial health line immediately
setTimeout(() => {
  logStructuredHealth(orch, governor, symbol);
    ...
    try {
      ...
      try {
        ...
        for await (const bar of alpacaFeed.subscribeBars(symbol)) {
          // STAGE 6: In QUIET mode, skip trading decisions but continue health logging
          if (governor.getMode() !== "ACTIVE") {
            // Explicit: QUIET mode skips bar processing (no trading decisions)
            // But health logging continues (handled by separate timer)
            continue;
          }
