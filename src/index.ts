...
// STAGE 3: Structured HB/status-pulse tracker
let bars1mCount = 0;
let bars5mCount = 0;
...
function logStructuredHB(orch: Orchestrator, governor: MessageGovernor, symbol: string): void {
...
// STAGE 3: Start structured HB/status-pulse timer (every 60 seconds, runs in all modes)
const symbol = process.env.SYMBOLS?.split(",")[0]?.trim() || "SPY";
setInterval(() => {
  logStructuredHB(orch, governor, symbol);
}, 60000); // 60 seconds

// Log initial HB/status-pulse immediately
setTimeout(() => {
  logStructuredHB(orch, governor, symbol);
    ...
    try {
      ...
      try {
        ...
        for await (const bar of alpacaFeed.subscribeBars(symbol)) {
          // STAGE 6: In QUIET mode, skip trading decisions but continue HB logging
          if (governor.getMode() !== "ACTIVE") {
            // Explicit: QUIET mode skips bar processing (no trading decisions)
            // But HB logging continues (handled by separate timer)
            continue;
          }
