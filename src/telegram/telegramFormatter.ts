...
export function buildTelegramAlert(snapshot: TelegramSnapshot): TelegramAlert | null {
  ...
  if (snapshot.type === "WATCH") {
    if (snapshot.range) {
      ...
      const rangeMode = snapshot.range.mode === "TIGHT" ? "CHOP" : "RANGE";
      const header = `${snapshot.symbol} ⚪ ${rangeMode} | WATCH | px ${formatPrice(snapshot.range.price)} | risk=${snapshot.risk}${timeSuffix}`;
      ...
      const arm = `ARM: ${snapshot.range.longArm ?? "retest range"} (bot creates play)`;
